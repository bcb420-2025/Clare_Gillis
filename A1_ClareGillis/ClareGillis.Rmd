...
sample_type_df = as.data.frame(sample_type, stringsAsFactors = FALSE)

# Apply TMM to the data
d = edgeR::DGEList(counts=filtered_data_matrix, 
            groups = factor(sample_type_df$layer, sample_type_df$diagnosis))
...
unique_groups <- unique(paste(sample_type_df$layer, sample_type_df$diagnosis))
color_map <- setNames(rainbow(length(unique_groups)), unique_groups)

# Generate MDS plot with colors by layer & diagnosis
limma::plotMDS(d, 
               labels = NULL, 
               pch = 1, 
               col = color_map[paste(sample_type_df$layer, sample_type_df$diagnosis)])
...
write.table(clean_data, file = "GSE251939_counts_clean_norm.txt", sep = "\t", 
